...
```{r}
input_data <- read_excel(here::here("datasets", "batData", "data", "raw", "bat_data_Bg_NT_v2.xlsx"))
```
...
```{r}
occurrence %<>% mutate(dwc_eventDate = format(dmy(`date (DD/MM/YYYY)`), "%Y-%m-%d"))
...
```{r}
occurrence %<>% mutate(dwc_decimalLatitude = `latitude (decimal degrees, YY.YYYYY)`) 
```

### decimalLongitude

```{r}
occurrence %<>% mutate(dwc_decimalLongitude = `longitude (decimal degrees, XX.XXXXX)` ) 
```
